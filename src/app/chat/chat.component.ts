...
import {
  A2HSDialogComponent
} from '../chat/a2-hsdialog/a2-hsdialog.component';
import {
  MatDialog,
  MatDialogConfig
...
} from '../credit-award/credit-award.component';
import {
  PreferenceWideningComponent
} from '../preference-widening/preference-widening.component';
import {
  timeout,
  retry,
  catchError,
...
} from '../subscriptionservice.service';
import {
...
export class ChatComponent implements OnInit, AfterViewInit {
          ...
          if (data.buttons.match('History') && data.buttons.length === 7 && !data.apiwha_autoreply.match('6')) {
            ...
            this.botui.message.add({
              ...
              if (data.buttons.match('Yes')) {
                ...
                return this.botui.action.button({
                  action: [{
                      text: this.changeBtnTextLanguage(this.currentLanguage, 'See Plans'),
                      value: 'Buy'
                    },
                    {
                      text: this.changeBtnTextLanguage(this.currentLanguage, 'Next Match'),
                      value: 'No'
                    }
                  ]
                }).then(res => {
  ...
  renewProfile() {
    return this.botui.action.button({
      action: [{
          text: this.changeBtnTextLanguage(this.currentLanguage, 'Renew Plan'),
          value: 'Renew'
        },
        {
          text: this.changeBtnTextLanguage(this.currentLanguage, 'Call Hans Care'),
          value: 'Call'
        }
      ]
    }).then(res => {
  ...
  changeLanguage(phon: string, lang: string): Observable < any > {
    ...
    return this.http.get < any > (' https://partner.hansmatrimony.com/api/language', {
      params: {
        ['phone_number']: phon,
        ['language']: lang
      }
      ...
      (data: any) => {
        ...
        if (data && data.family && data.family.caste) {
          this.http.get < {
            mapping_id: number,
            castes: any
          } > ('https://partner.hansmatrimony.com/api/caste_mapping').subscribe(
            res => {
              this.casteMap = res;

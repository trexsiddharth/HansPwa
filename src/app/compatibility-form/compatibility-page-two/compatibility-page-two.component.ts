...
firstStep() {
    ...
    if (this.PageTwo.valid) {
      ...
      this.casteValidation(this.PageTwo.value.Castes).then(res => {
          if (res === true) {
            if (this.PageTwo.valid) {
              this.spinner.show();
              const date = this.PageTwo.value.birth_date;
              const month = this.month.indexOf(this.PageTwo.value.birth_month) + 1;
              const year = this.PageTwo.value.birth_year;
            ...
            // tslint:disable-next-line: max-line-length
              ...
              if (res.status === 1) {
                ...
                localStorage.setItem('gender', this.PageTwo.value.gender);
                localStorage.setItem('mobile_number', this.PageTwo.value.phone);
                ...
                (window as any).fbq('track', 'SinglePageRegistration', {
                  value: 15,
                  currency: 'INR',
                  content_name: this.PageTwo.value.phone,
                });
...
async casteValidation(value) {
  ...
  if (statusConfirmed === false) {
      ...
      this.PageTwo.get('Castes').setValue('');
      return false;
    }
  return true;
